...
class AuthState 
{
  ...
  const AuthState({this.user, this.isLoading = false, this.errorMessage,this.isAuthenticated = false});

  AuthState copyWith({UserModel? user, bool? isLoading, String? errorMessage, bool? isAuthenticated}) 
  {
    return AuthState(
      user: user ?? this.user,
...
class AuthNotifier extends StateNotifier<AuthState> 
{
  ...
  AuthNotifier(this._authRepository) : super(const AuthState()) {_authRepository.authStateChanges.listen(_onAuthStateChanged);}
  ...
  Future<void> _onAuthStateChanged(User? firebaseUser) async 
  {
    ...
    try 
    {
      ...
      if (userModel != null) {state = AuthState(
        user: userModel, 
        isAuthenticated: true
      );} 
      else {state = const AuthState(isAuthenticated: false);}
    } 
    catch (e) 
    {
  ...
  Future<void> signInWithGoogle({required String role}) async 
  {
    ...
    on AuthCancelledException {state = state.copyWith(isLoading: false);} 
    catch (e) 
    {
