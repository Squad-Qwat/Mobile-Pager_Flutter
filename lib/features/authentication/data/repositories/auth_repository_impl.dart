...
class AuthRepositoryImpl implements IAuthRepository 
{
  ...
  Future<UserModel> signInAsGuest() async 
  {
    try 
    {
      ...
      if (user == null) {throw Exception('User is null after anonymous sign in');}
  ...
  {
    try {await _remoteDataSource.signOut();} 
    catch (e) {rethrow;}
  }
  ...
  @override
  Future<void> updateUserProfile({required String uid,String? displayName,String? photoURL}) async 
  {
    try 
    {
      ...
      if (updates.isNotEmpty) {await _remoteDataSource.updateUserDocument(uid, updates);}
    } 
    catch (e) {rethrow;}
  }
  ...
  Future<UserModel> linkAnonymousToGoogle({required String role}) async 
  {
    try 
    {
      ...
      if (currentFirebaseUser == null) {throw Exception('No user signed in');}

      if (!currentFirebaseUser.isAnonymous) {throw Exception('Current user is not anonymous');}

      final credential = await _remoteDataSource.getGoogleCredential();
      final linkedUser = await currentFirebaseUser.linkWithCredential(credential,);
      final user = linkedUser.user;

      if (user == null) {throw Exception('Failed to link account');}
      ...
      await _remoteDataSource.setUserDocument(
        user.uid, 
        updatedUser.toMap()
      );
