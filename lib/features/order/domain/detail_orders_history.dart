...
// This was originally in 'orders_filter_widget.dart'
...
class DetailOrdersHistoryPage extends StatefulWidget 
{
  final String orderId;

  const DetailOrdersHistoryPage({Key? key, required this.orderId}) : super(key: key);

  @override
  State<DetailOrdersHistoryPage> createState() => _DetailOrdersHistoryPageState();
}

class _DetailOrdersHistoryPageState extends State<DetailOrdersHistoryPage> 
{
  Orders? _order;
  ...
  void _loadOrderDetail() 
  {
    setState(() 
    {
      // Find the order from the 'recentActivitiesData' list
      try {_order = recentActivitiesData.firstWhere((order) => order.orderId == widget.orderId,);} 
      // Order not found
      catch (e) {_order = null;}
    });
  }
          ...
          children: <Widget>[
            ...
            if (_order!.customer.name != null || _order!.customer.phone != null) ...[
              _buildInfoCard(
                ...
                children: <Widget>[
                  if (_order!.customer.name != null) ...[
                    _buildInfoRow(
                      'Nama',
                      _order!.customer.name ?? 'guest', //
                    )
                  ],
                  if (_order!.customer.phone != null) ...[
                    _buildInfoRow(
                      'No. Telepon',
                      '${_order!.customer.phone ?? 00000000000}', //
                    )
                  ],
                  if (_order!.customer.tableNumber != null) ...[
                    _buildInfoRow(
                      'Nomor Meja',
                      '${_order!.customer.tableNumber ?? 0}', //
                    )
                  ],
                ],
              ),
  ...
  Future<void> _confirmPickup(Orders order) async 
  {
    ...
    if (confirm == true) 
    {
      ...
      // In a real app, you would update the order state here
      // and then call _loadOrderDetail() to refresh
    }
  }
