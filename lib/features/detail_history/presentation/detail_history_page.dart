...
class _DetailHistoryPageState extends State<DetailHistoryPage> {
  ...
  void _loadOrderDetail() {
    setState(() {
      _order = DummyDataService.getDummyOrderDetail(widget.orderId);
    });
  }
              ...
              child: Column(
                children: [
                  ...
                  _buildInfoCard(
                    ...
                    children: [
                      _buildInfoRow('Order ID', _order!.orderId),
                      _buildInfoRow(
                        'Nomor Antrian',
                  ...
                  if (_order!.customer.name != null ||
                      _order!.customer.phone != null)
                    _buildInfoCard(
                      title: 'Informasi Pelanggan',
                      children: [
                        if (_order!.customer.name != null)
                          _buildInfoRow('Nama', _order!.customer.name!),
                        if (_order!.customer.phone != null)
                          _buildInfoRow('No. Telepon', _order!.customer.phone!),
  ...
  Widget _buildHeaderCard(Orders order) {
    return Container(
      ...
      child: Column(
        children: [
          Icon(_getStatusIcon(order.status), size: 64, color: Colors.white),
        ...
        children: [
          ...
          _buildTimelineItem(
            ...
            subtitle: order.readyAt != null
                ? 'Waktu tunggu: ${order.getWaitingTime()}'
                : null,
          ),
          _buildTimelineItem(
            title: 'Sudah Diambil',
  ...
  Future<void> _orderAgain(Orders order) async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Fitur pesan lagi akan segera hadir!')),
    );
  }
}
