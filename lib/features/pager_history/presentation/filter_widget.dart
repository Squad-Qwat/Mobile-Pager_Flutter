...
class HistoryFilterWidget extends StatefulWidget 
{
  ...
  const HistoryFilterWidget({Key? key, required this.currentOptions, required this.onFilterChanged}) : super(key: key);
...
{
  late HistoryFilterOptions _options;
  ...
  Widget _buildSearchBar() 
  {
    return TextField(
      onChanged: (value) 
      {
        setState(() {_options = _options.copyWith(searchQuery: value);});
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                HistoryFilterService.getTimeFilterLabel(_options),
  ...
  Widget _buildStatusFilterButton() 
  {
    return PopupMenuButton<List<String>>(
      ...
      onSelected: (value) 
      {
        setState(() {_options = _options.copyWith(statusFilter: value);});
        widget.onFilterChanged(_options);
      },
  ...
  Widget _buildSortButton() 
  {
    return PopupMenuButton<SortOrder>(
      ...
      onSelected: (value) 
      {
        setState(() {_options = _options.copyWith(sortOrder: value);});
        widget.onFilterChanged(_options);
      },
  ...
  Widget _buildTimeFilterOption(String label, TimeFilter filter) 
  {
    return ListTile(
      ...
      onTap: () async 
      {
        ...
        if (filter == TimeFilter.customRange) {await _showDateRangePicker();} 
        else if (filter == TimeFilter.customMonthYear) {await _showMonthYearPicker();} 
        else 
        {
          setState(() {_options = _options.copyWith(timeFilter: filter);});
          widget.onFilterChanged(_options);
        }
      },
    );
  ...
  Future<void> _showDateRangePicker() async 
  {
    ...
    {
      setState(() 
      {_options = _options.copyWith(timeFilter: TimeFilter.customRange, startDate: picked.start, endDate: picked.end,);});
      widget.onFilterChanged(_options);
    }
  }
          ...
          content: Column(
            ...
            children: <Widget>[
              ...
              DropdownButtonFormField<int>(
                ...
                items: List.generate(5, (index) => now.year - index).map((year) => DropdownMenuItem(
                  value: year, 
                  child: Text(
                    '$year', 
                    style: GoogleFonts.inter()
              ...
              DropdownButtonFormField<int>(
                ...
                items: List.generate(12, (index) => index + 1).map((month) => DropdownMenuItem(
                  value: month,
                  child: Text(
                    _getMonthName(month),
                    style: GoogleFonts.inter(),
