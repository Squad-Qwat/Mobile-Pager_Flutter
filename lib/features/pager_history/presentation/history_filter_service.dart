...
class HistoryFilterService 
{
  ...
  static List<History> _filterByTime(List<History> historyList, HistoryFilterOptions options) 
  {
    ...
    switch (options.timeFilter) 
    {
      ...
      case TimeFilter.thisWeek:
        ...
        return historyList
            .where((h) => h.createdAt.isAfter(startOfWeekDay))
            .toList();
      ...
      case TimeFilter.customRange:
        if (options.startDate != null && options.endDate != null) {
          ...
          return historyList
              .where((h) =>
                  h.createdAt.isAfter(start) && h.createdAt.isBefore(end))
              .toList();
        }
        return historyList;
      ...
      case TimeFilter.customMonthYear:
        if (options.month != null && options.year != null) {
          return historyList
              .where((h) =>
                  h.createdAt.month == options.month &&
                  h.createdAt.year == options.year)
              .toList();
        }
        return historyList;
  ...
  static List<History> _filterBySearch(List<History> historyList, String query) 
  {
    ...
    return historyList.where((h) 
    {
      ...
      final matchesBusinessName =
          h.businessName?.toLowerCase().contains(lowerQuery) ?? false;
